...
const makeListCategoriesFn = (
  ...
  return async () => {
    ...
    try {
      ...
      console.info(
        "Category metadata cache file not found, creating a new one."
      );
    }
    ...
    for (const chunk of chunkedPartialCategories) {
      const completeCategoryPromises = chunk.map(async (category) => {
        ...
        if (
          cachedMetadata?.find(
            (meta) => meta.key === "ssactivewear_category_id"
          )
        ) {
          console.info(`Using cached data for category ${category.id}`);
          ...
          if (error) {
            console.error("Error fetching category metadata", {
              context: { error },
            });
